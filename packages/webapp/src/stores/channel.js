  ...
  actions: {
    loadChannel: async ({ state, dispatch, rootState, commit }) => {
      ...
      if (channel.needsInitialSig) {
        ...
        await commit(
          'logNormal',
          { append: 'Creating state channel......done' },
          { root: true }
        )
      } else {
        await commit(
          'logNormal',
          { append: 'Loading state channel......done' },
          { root: true }
        )
      }
    ...
    loadBalance: async ({ state, rootState, dispatch }) => {
      ...
      const channelId = getChannelId(state.latestState.channel)
      const balance = await rootState.contract.blsMove.holdings(
    ...
    deposit: async ({ state, rootState, dispatch }) => {
      ...
      const keyIndex = await rootState.contract.blsMove.indexByKey(
